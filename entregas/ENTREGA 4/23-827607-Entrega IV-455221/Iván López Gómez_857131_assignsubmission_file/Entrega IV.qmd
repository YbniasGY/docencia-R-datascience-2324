---
title: "Entrega IV"
author: "Ivan Lopez - DNI: 51496696-H"
format:
  html:
    theme: [style.scss]
    toc: true
...
clabels <- c("ene", "feb", "mar", "abr", "may", "jun", "jul", "ago", "sep", "oct", "nov", "dic")


flights |> 
ggplot(aes(x= month)) + 
  geom_bar(aes(x = factor(month, labels = clabels), fill = factor(month, labels = clabels))) +
  theme_minimal() +
  labs(x = "Mes", y = "Número de vuelos", fill= 'month',title = "Diagrama de barras", caption = "J. Álvarez Liébana")+
...
ggplot(aes(x= month)) + 
  ...
  theme(
    ...
    plot.title = element_text(name='Merriweather', size = 30),
...
result <- data |> 
  ...
  summarise(media = mean(Probability), .by = Términos)


  ggplot(result)+
  geom_col(aes(x=Términos, y = media, fill = media), alpha=1)+
...
ggplot(result) +
  geom_col(aes(x = Category, y = media, fill = media), alpha = 1) +
  scale_fill_gradient2(low = "#de5853", high = "#50709e",
                        mid = "#fdf8e4", midpoint = 50) +
  theme(legend.position = "bottom") +
  theme_minimal() +
...
datad <- read_csv("https://raw.githubusercontent.com/zonination/perceptions/master/probly.csv")

datos_filtrados <- datad |>
